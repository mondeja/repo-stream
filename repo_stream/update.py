    ...
    for arg in args:
        if _next_is_config:
            ...
            if arg_without_script == "config":
                ...
                response[argname] = (repo_url_to_full_name(value)
                                     if argname == "config" else value)
                _next_is_config = False
                _next_is_updater = False
    ...
    for repo in repos:
        ...
        for file in tree_res["tree"]:
            if file["path"] == ".pre-commit-config.yaml":
                ...
                file_req = urllib.request.urlopen(repo_tree_url)
...
def get_stream_config_pre_commit_configurations(repos_stream_config):
    for i, repo in enumerate(repos_stream_config):
        repos_stream_config[i]["updater_content"] = (
            get_stream_config_file_content(
                repo['config'],
